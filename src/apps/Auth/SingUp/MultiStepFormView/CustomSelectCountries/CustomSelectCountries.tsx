...
const CustomSelectCountries = ({
  ...
  return (
    ...
    >
      <Search className="absolute inset-y-0 right-10 h-full" />
      <Polygon
        className={`absolute inset-y-0 right-4 h-full transition-all duration-700 ${
      ...
      <input
        ...
        className="w-full h-full outline-none p-[8px] pr-[42px] bg-transparent rounded-[10px] text-[--text-body] text-base focus:border-[1.5px] focus:border-[--yellow]"
        onFocus={() => setIsFocus(true)}
        autoComplete="off"
        disabled={disabled}
      />
      <ul
        className={`select bg-[--dark-gray] w-[50%] mt-2 px-2 overflow-y-auto rounded-[10px] absolute z-50 transition-all duration-300 space-y-2 ${
